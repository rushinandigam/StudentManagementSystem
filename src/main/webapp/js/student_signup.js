...
// Regex patterns
const patterns = {
    registerNumber:  /^\d{2}[kK]6(1[aA]|[kK]6|5[aA])12(\d{2}|[A-Za-z]\d)$/,
    name: /^[A-Za-z\s]{3,50}$/,
...
window.onload = () => {
    attachBlurValidation("registerNumber", patterns.registerNumber, "Invalid format (e.g., 23K61A1286)");
    ...
    attachBlurValidation("address", patterns.address, "Address must be 10–100 characters");
...
function validateStudentSignupForm() {
    ...
    const validations = [
        { id: "registerNumber", valid: patterns.registerNumber.test(inputs.registerNumber), msg: "Invalid format" },
        ...
        { id: "address", valid: patterns.address.test(inputs.address), msg: "Invalid address" },
